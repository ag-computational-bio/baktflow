...
def parse_gecco(genes: str, features: str, cluster: str, sample_name: str):
    ...
    columns_genes = [
        ...
        "strand", "average_p", "max_p"
    ]

    columns_features = [
        "sequence_id", "protein_id", "start", "end",
        "strand", "domain", "hmm", "i_evalue",
        "p_value", "domain_start", "domain_end",
        "cluster_probability"
    ]

    columns_cluster = [
        "sequence_id", "cluster_id", "start", "end",
        "average_p", "max_p", "type",
    ...
    if features is not None:
        try:
            df_features = pl.read_csv(
                features,
                separator="\t",
                new_columns=columns_features
            )
        except pl.exceptions.NoDataError:
            pass
