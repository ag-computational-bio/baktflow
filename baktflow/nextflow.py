...
def baktflow_setup(
    ...
    if not os.path.exists(nextflow_path):
        ...
        raise Exception(f"Could not find nextflow executable at {nextflow_path}. Please provide a valid path.")

    if not os.path.exists(setup_script):
        logger.error(f"Cannot find script file: {setup_script}")
        raise Exception(f"Could not find nextflow setup script at {setup_script}")
    ...
    try:
        ...
        raise Exception(f"Nextflow setup script failed: {e}")
